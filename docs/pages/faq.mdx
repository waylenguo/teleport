...
Yes, Teleport supports tunnel multiplexing on a single port. Set the `tunnel_listen_addr` option to use the same port as the `web_listen_addr` address setting in the `proxy_service` configuration. Teleport will automatically use multiplexing with that configuration.
