...
func (process *TeleportProcess) getConnectorForRole(role types.SystemRole) (conn *Connector, ok bool) {
	process.Lock()
	defer process.Unlock()
	conn, ok = process.connectors[role]
	return
}
...
func (process *TeleportProcess) getInstanceRoleEventMapping() map[types.SystemRole]string {
	process.Lock()
	defer process.Unlock()
	out := make(map[types.SystemRole]string)
...
// enableInstanceRole marks a given instance role as active, storing the name of its associated
// identity event.
func (process *TeleportProcess) enableInstanceRole(role types.SystemRole, eventName string) {
	process.Lock()
	defer process.Unlock()
	process.instanceRoles[role] = eventName
}

func (process *TeleportProcess) instanceRoleEnabled(role types.SystemRole) bool {
	process.Lock()
	defer process.Unlock()
...
func NewTeleport(cfg *Config) (*TeleportProcess, error) {
	...
	componentCount := process.registerExpectedServices(cfg)
	...
	process.inventoryHandle = inventory.NewDownstreamHandle(process.makeInventoryControlStream, proto.UpstreamInventoryHello{
	...
	process.inventoryHandle.RegisterPingHandler(func(sender inventory.DownstreamSender, ping proto.DownstreamInventoryPing) {
		log.Infof("Handling incoming inventory ping (id=%d).", ping.ID)
		err := sender.Send(context.TODO(), proto.UpstreamInventoryPong{
			ID: ping.ID,
		})
		if err != nil {
			log.Warnf("Failed to respond to inventory ping (id=%d): %v", ping.ID, err)
		}
	})
...
func (process *TeleportProcess) registerWithAuthServer(role types.SystemRole, eventName string) {
	...
	process.RegisterCriticalFunc(fmt.Sprintf("register.%v", serviceName), func() error {
		if role.IsLocalService() && !process.instanceRoleEnabled(role) {
			// if you hit this error, your probably forgot to call enableInstanceRole inside of
			// the registerExpectedServices function.
			return trace.Errorf("register called for instance role that has not been enabled (this is a bug)")
		}
...
// registerExpectedServices ensures that a TeleportReadyEvent is produced
// when all components enabled (based on the configuration) have started, and
// sets up the instance role -> identity event mapping. The number of services
// is returned (note: the returned count is based on the number of expected ready
// events rather than the number of expected identities. these may differ if the
// old style proxy-based kube service is used since that servce shares the proxy
// identity rather than having its own unique identity).
func (process *TeleportProcess) registerExpectedServices(cfg *Config) int {
	eventMapping := EventMapping{
		Out: TeleportReadyEvent,
	}

	if cfg.Auth.Enabled {
		process.enableInstanceRole(types.RoleAuth, AuthIdentityEvent)
		eventMapping.In = append(eventMapping.In, AuthTLSReady)
	}

	if cfg.SSH.Enabled {
		process.enableInstanceRole(types.RoleNode, SSHIdentityEvent)
		eventMapping.In = append(eventMapping.In, NodeSSHReady)
	}

	proxyConfig := cfg.Proxy
	if proxyConfig.Enabled {
		process.enableInstanceRole(types.RoleProxy, ProxyIdentityEvent)
	...
	if cfg.Kube.Enabled {
		process.enableInstanceRole(types.RoleKube, KubeIdentityEvent)
		eventMapping.In = append(eventMapping.In, KubernetesReady)
	}

	if cfg.Apps.Enabled {
		process.enableInstanceRole(types.RoleApp, AppsIdentityEvent)
		eventMapping.In = append(eventMapping.In, AppsReady)
	}

	if cfg.Databases.Enabled {
		process.enableInstanceRole(types.RoleDatabase, DatabasesIdentityEvent)
		eventMapping.In = append(eventMapping.In, DatabasesReady)
	}

	if cfg.WindowsDesktop.Enabled {
		process.enableInstanceRole(types.RoleWindowsDesktop, WindowsDesktopIdentityEvent)
