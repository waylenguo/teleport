...
func (s *Server) HandleConnection(conn net.Conn) {
	...
	// a little differently than "normal" HTTP requests from web apps.
	if app.IsTCP() {
		if err := s.handleTCPApp(s.closeContext, tlsConn, id, app); err != nil {
...
func (s *Server) handleTCPApp(ctx context.Context, clientConn net.Conn, identity *tlsca.Identity, app types.Application) error {
	addr, err := utils.ParseAddr(app.GetURI())
	if err != nil {
		return trace.Wrap(err)
	}
	if addr.AddrNetwork != "tcp" {
		return trace.BadParameter("address network is not supported: %+v", addr)
	}
