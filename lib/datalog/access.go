...
// extern output_t *process_access(const uint8_t *, size_t);
// extern unsigned char *output_access(const output_t *);
// extern size_t output_length(const output_t *);
// extern int output_error(const output_t *);
// extern void drop_output_struct(output_t *);
import "C"
...
func ProcessAccess(input []byte) ([]byte, string) {
	...
	ptr := C.output_access(output)
	size := C.output_length(output)
	if size > C.INT_MAX {
		return nil, "output size overflow"
	}
	len := C.int(size)

	if C.output_error(output) != 0 {
		if len == 0 {
