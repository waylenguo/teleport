...
func (a *Server) checkTokenJoinRequestCommon(ctx context.Context, req *types.RegisterUsingTokenRequest) (types.ProvisionToken, error) {
	...
	if err != nil {
		...
		return nil, trace.AccessDenied(fmt.Sprintf("%q [%v] can not join the cluster with role %s, %s", req.NodeName, req.HostID, req.Role, msg))
	}
	...
	if req.Role == types.RoleInstance {
		...
		if !hasLocalServiceRole {
			msg := fmt.Sprintf("%q [%v] cannot requisition instance certs (token conains to local service roles)", req.NodeName, req.HostID)
			log.Warn(msg)
...
func (a *Server) generateCerts(ctx context.Context, provisionToken types.ProvisionToken, req *types.RegisterUsingTokenRequest) (*proto.Certs, error) {
	...
	// instance certs include an additional field the specifies the list of
