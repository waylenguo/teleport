...
type DownstreamInventoryControlStream interface {
	...
	// the stream is open, or if the the stream was closed explicitly via the Close()
	// method).
	Error() error
}
...
type UpstreamInventoryControlStream interface {
	...
	// Close closes the underlying stream with optional error.
	Close(...error)
	// Done signals that the stream has been closed.
	Done() <-chan struct{}
	// Error checks for any error associated with stream closure (returns `nil` if
	// the stream is open, or if the stream was closed explicitly via the Close()
	// method).
	Error() error
}
...
func InventoryControlStreamPipe() (UpstreamInventoryControlStream, DownstreamInventoryControlStream) {
	pipe := &pipeControlStream{
		downC: make(chan proto.DownstreamInventoryMessage, 1),
		upC:   make(chan proto.UpstreamInventoryMessage, 1),
...
func (p *pipeControlStream) Close(e ...error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	if !p.closed {
		if len(e) != 0 {
...
// UpstreamInventoryHello, and the first message recieved must be a DownstreamInventoryHello.
...
func (i *downstreamICS) runRecvLoop(stream proto.AuthService_InventoryControlStreamClient) {
	for {
		...
		if err != nil {
			i.closeWithError(trace.Errorf("inventory control stream closed: %v", trail.FromGRPC(err)))
			return
		}
		...
		case oneOf.GetPing() != nil:
			msg = *oneOf.GetPing()
		default:
			log.Warnf("Ignoring unknown downstream inventory msg variant: %T", oneOf.GetMsg())
			continue
		}
...
func (i *downstreamICS) runSendLoop(stream proto.AuthService_InventoryControlStreamClient) {
	for {
		...
		case sendMsg := <-i.sendC:
			...
			sendMsg.errC <- err
			if err != nil {
				i.closeWithError(trace.Errorf("inventory control stream closed: %v", err))
				return
			}
		case <-i.Done():
			// stream closed by other goroutine
...
func (i *downstreamICS) closeWithError(err error) {
	i.mu.Lock()
	defer i.mu.Unlock()
	if i.closed {
		return
...
func (i *downstreamICS) Close(e ...error) {
	var err error
	if len(e) != 0 {
		err = e[0]
	}
...
func (i *upstreamICS) closeWithError(err error) {
	i.mu.Lock()
	defer i.mu.Unlock()
	if i.closed {
		return
...
func (i *upstreamICS) Close(e ...error) {
	var err error
	if len(e) != 0 {
		err = e[0]
	}
